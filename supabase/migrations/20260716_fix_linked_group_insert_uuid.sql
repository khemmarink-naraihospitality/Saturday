-- Fix: INSERT into a linked group always failed because the trigger calls
-- uuid_generate_v4(), which lives in the `extensions` schema, while the
-- function pins `search_path = public`. Every insert into a linked group
-- aborted with "function does not exist", reverting the user's new item.
-- Switch to gen_random_uuid() (pg_catalog, always resolvable).
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    v_new_mirror_id := gen_random_uuid();
  ...
  if tg_op = 'UPDATE' then
    ...
    -- semantics already used for values/title.
    update items set title = new.title, values = v_mirrored_values, updates = coalesce(new.updates, '[]'::jsonb), is_hidden = coalesce(new.is_hidden, false)
