-- Fix the "View Item" link in due-date reminder emails: it was hardcoded
-- to the bare app URL ('https://saturdaycom.vercel.app') with no boardId
-- or itemId, so clicking it just landed on the homepage instead of the
-- item. Every other transactional email (assignment, in memberSlice.ts)
-- already builds the link as `?boardId=...&itemId=...`, which App.tsx's
-- deep-link handler picks up on load to open that item directly — and
-- since TaskDetail defaults to its "Updates" tab, opening the item is
-- enough to land the user on Updates.
--
-- Only the itemLink line changes; the rest of the function body is
-- unchanged from the 20260818 version.
        ...
        if assignee_email is not null then
          perform net.http_post(
            ...
            body := jsonb_build_object(
              ...
              'dueLabel', due_label,
              'itemLink', 'https://saturdaycom.vercel.app/?boardId=' || rec.board_id || '&itemId=' || rec.item_id
            )
          );
        end if;
      end if;
...
-- database that runs only this file still ends up scheduled. Schedule
-- matches 20260821_reminder_morning_bangkok.sql (01:00 UTC = 08:00 Bangkok).
